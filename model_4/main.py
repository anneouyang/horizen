...
def main():
	...
	while chain.time <= MAX_TIME:
		if(nodes[0].get_ds_block() != chain.blocks[0]):
			nodes[0].isBad = True
		if(nodes[1].get_ds_block() != chain.blocks[0]):
			nodes[1].isBad = True
		if(nodes[2].get_ds_block() != chain.blocks[0]):
			nodes[2].isBad = True
	...
	print(nodes[0].attackSuccess)
	print(nodes[1].attackSuccess)
	print(nodes[2].attackSuccess)
	# chain.print_chain()
